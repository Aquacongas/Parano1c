...
//! Stage D.2 baseline — the honest realistic fixture still accepts.
//!
//! Historically this file exhaustively tampered the Merkle-side absorb
//! pin chain (`o1_prog[lane]`, `pre_s[lane]`, `payload[lane]`) at every
//! `InputLeafPermA` / `InputLeafPermB` row and asserted each tamper
//! rejected. Those cells are gone — the 59-perm Merkle band is retired
//! and GKR owns the permutation soundness. Input-lane binding is now
//! closed by the row-wide `tx_body_hash` pin on
