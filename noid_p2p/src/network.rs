...
fn android_p2p_log(message: impl AsRef<str>) {
    use std::ffi::CString;
    ...
    let Ok(tag) = CString::new("NOID_P2P") else {
        return;
    };
...
#[inline]
fn android_p2p_log(_message: impl AsRef<str>) {}
...
async fn run_swarm(
    ...
    android_p2p_log("REACTOR 01 run_swarm ENTER");
    ...
    android_p2p_log("REACTOR 02 chain.read BEGIN");

    let chain_store = {
        let ctx = chain.read().await;

        android_p2p_log("REACTOR 03 chain.read ACQUIRED");

        ctx.store.clone()
    };

    android_p2p_log("REACTOR 04 chain.read RELEASED");
    ...
    android_p2p_log("REACTOR 05 SwarmBuilder BEGIN");

    let builder = SwarmBuilder::with_existing_identity(identity).with_tokio();

    android_p2p_log("REACTOR 06 with_tokio OK");

    let builder = builder.with_tcp(
        tcp::Config::default().nodelay(true),
        noise::Config::new,
        yamux::Config::default,
    )?;

    android_p2p_log("REACTOR 07 with_tcp OK");

    #[cfg(not(target_os = "android"))]
    let builder = {
        let builder = builder.with_dns()?;
        android_p2p_log("REACTOR 08 with_dns OK");
        builder
    };
    ...
    let builder = {
        android_p2p_log(
            "REACTOR 08 Android DNS transport skipped; bootstrap uses resolved IP addresses",
        );
        builder
    };

    let builder = builder.with_relay_client(noise::Config::new, yamux::Config::default)?;

    android_p2p_log("REACTOR 09 with_relay_client OK");

    let builder = builder.with_behaviour(move |key, relay_client| {
        android_p2p_log("REACTOR 10 NodeBehaviour::new BEGIN");

        let result = NodeBehaviour::new(
            key,
            &protocol_id,
            relay_client,
            background_capacity,
        ...
        android_p2p_log("REACTOR 11 NodeBehaviour::new RETURN");

        result
    })?;

    android_p2p_log("REACTOR 12 with_behaviour OK");
    ...
    android_p2p_log("REACTOR 13 swarm build OK");
...
fn begin_bootstrap_dial(
    ...
    match swarm.dial(options) {
        Ok(()) => {
            android_p2p_log(format!(
                "DIAL START {} connection={:?}",
                addr, connection_id
            ));
        ...
        Err(error) => {
            android_p2p_log(format!(
                "DIAL REJECT {} connection={:?} error={}",
                addr, connection_id, error
            ));
...
async fn handle_network_command(
    ...
    match cmd {
        ...
        NetworkCommand::Dial { addr } => {
            android_p2p_log(format!("SEED REGISTER {}", addr));
...
async fn handle_swarm_event(
    ...
    match event {
        ...
        SwarmEvent::ConnectionEstablished {
            ...
            android_p2p_log(format!(
                "CONNECTED peer={} connection={:?} remote={}",
                peer_id,
                connection_id,
        ...
        SwarmEvent::ConnectionClosed {
            ...
            android_p2p_log(format!(
                "CLOSED peer={} connection={:?} remaining={} cause={:?}",
        ...
        SwarmEvent::OutgoingConnectionError {
            ...
            android_p2p_log(format!(
                "DIAL ERROR peer={:?} connection={:?} error={}",
