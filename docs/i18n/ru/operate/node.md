...
P2P-соединения принимаются на TCP `9500`. JSON-RPC должен оставаться на
`127.0.0.1:9501`.
...
Разрешите входящий TCP `9500` в межсетевых экранах сервера и провайдера. За
NAT перенаправьте TCP `9500` на ноду. Синхронизация работает и при наличии только
исходящих соединений, но входящие пиры делают ноду полезной для сети.

Не публикуйте TCP `9501`. Для удалённого администрирования используйте
