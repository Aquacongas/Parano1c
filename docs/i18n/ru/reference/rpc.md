...
При отсутствующем или другом токене сервер отвечает HTTP `401` без
JSON-RPC-результата. Правильный mining-токен разрешает только
`paranoid_getBlockTemplate` и `paranoid_submitBlock`; он не даёт доступа к
кошельку, управлению нодой и остальным RPC-методам. Токен аутентифицирует, но
не шифрует соединение; удалённо используйте приватный транспорт или TLS-прокси.

Передача токена в командной строке сохранена для совместимости. Предпочтителен
файл с доступом только владельцу: он не показывает токен в аргументах процесса.
