...
节点通过 TCP `9500` 接受 P2P 连接。JSON-RPC 应保持绑定
`127.0.0.1:9501`。
...
在主机和服务商防火墙中允许入站 TCP `9500`。若服务器位于 NAT 后，将
TCP `9500` 转发给节点。节点只靠出站连接也能同步，但接受入站连接才能
更好地为网络服务。

不要公开 TCP `9501`。远程管理应使用 SSH 隧道或其他经过认证的私有
传输。
