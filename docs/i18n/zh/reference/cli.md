...
| `--p2p-listen HOST:PORT` | P2P 监听；默认 `0.0.0.0:9500` |
| `--rpc-listen HOST:PORT` | JSON-RPC 监听；默认 `127.0.0.1:9501` |
...
| 参数 | 默认值 | 含义 |
|---|---|---|
| `--rpc URL` | `http://127.0.0.1:9501` | 节点 JSON-RPC 端点 |
...
默认端点是 `http://127.0.0.1:9501`。环境变量 `NOID_RPC` 可修改；
`--rpc` 优先级更高。
