...
//! Stage G1b.β — chained product-sumcheck reduction for one Poseidon2b
//! permutation.
//!
...
//! from `state_in`.
//!
//! Caveat documented for G2: this stage does not yet include a
//! cross-row MDS consistency sumcheck. Mutation D (prover uses
//! `MDS_FULL` on a partial round) is still detected because it changes
//! `state[r+1]` onward, which makes the prover's state-MLE disagree
//! with the verifier's honestly reconstructed state-MLE — the
//! final `check_sin_reduction` rejects. Succinct cross-row binding is
//! the job of G2's batched spine sumcheck.

use noid_core::mle::evaluate::evaluate_slice;
use noid_core::transcript::FiatShamir;
use noid_core::{Block128, TowerField};
use noid_poseidon2b::native::permutation::{N_ROUNDS, ROUND_CONSTANTS, STATE_SIZE};

use crate::layers::{evaluate_permutation, round_kind, RoundKind};
use crate::mle_layout::{PermMle, N_PERM_CELLS, N_PERM_VARS};
use crate::product_sumcheck::{prove_product, verify_product, ProductProof};
...
/// Stage G3.γ₁ — output of a verified permutation sumcheck. The
/// sumcheck reduces the `v₀ = sout_mle(r₀)` claim all the way down
/// to three MLE-evaluation claims on the slot's `state` column. The
/// verifier does **not** discharge them anymore — it returns them
/// here so a caller (γ₂ batching) can reduce the whole 59-slot spine
/// to a single `(r_B, v_B)` pair against the concatenated boundary
/// MLE `B`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
// γ₄: `absorb_boundary(state_in)` deleted. `state_in` is a slice of
// the concatenated boundary MLE `B`, already bound through the outer
// `(r_B, v_B)` claim discharged by the STARK extras column.
...
/// Honest prover. Absorbs `state_in` into the transcript, squeezes the
/// claim point `r₀`, evaluates `sout_mle(r₀) = v₀`, runs the chain, and
/// returns the proof and `v₀` (so test callers can re-check it against
/// the native-reconstructed witness).
pub fn prove_perm<T: FiatShamir<Block128>>(
    state_in: [Block128; STATE_SIZE],
    channel: &mut T,
) -> (PermProof, Block128, [PermStateClaim; N_STATE_CLAIMS_PER_SLOT]) {
    ...
    let (p_x4, r2) = prove_product(&mle.x2, &mle.x2, &r1, p_sout.a_final, channel);
    let (p_x3, r3) = prove_product(&mle.x2, &mle.sin, &r1, p_sout.b_final, channel);
    let (p_x2a, r4) = prove_product(&mle.sin, &mle.sin, &r2, p_x4.a_final, channel);
    let (p_x2b, r5) = prove_product(&mle.sin, &mle.sin, &r3, p_x3.a_final, channel);

    // Sin expansion: sin(ρ) = Σ_x eq(ρ,x) · active(x) · (state(x) + rc(x)).
    let active_mle = build_active_mle();
    let rc_mle = build_rc_mle();
    ...
    let (p_sin_r3, rs3) = prove_product(&active_mle, &state_plus_rc, &r3, sin_at_r3, channel);
    let (p_sin_r4, rs4) = prove_product(&active_mle, &state_plus_rc, &r4, sin_at_r4, channel);
    let (p_sin_r5, rs5) = prove_product(&active_mle, &state_plus_rc, &r5, sin_at_r5, channel);
...
/// [`PermStateClaim`]). The caller (γ₂ spine batching) is responsible
/// for discharging those three claims against the actual boundary
/// MLE — `verify_perm` no longer reconstructs the state column.
///
/// Returns the three state-MLE claims on accept, `None` on reject.
///
/// `v0_sout` is the claimed `sout_mle(r0)` value that bootstraps the
/// chain. In γ₁ callers pass the natively-reconstructed value
/// (matching pre-γ₁ behaviour). In γ₂+ it'll come from the outer
/// batching sumcheck.
pub fn verify_perm<T: FiatShamir<Block128>>(
    proof: &PermProof,
    state_in: [Block128; STATE_SIZE],
    channel: &mut T,
    ...
    // γ₁ keeps `v0 = sout_mle(r0)` derived natively from `state_in`.
    // γ₂ will supply it from an outer batching sumcheck and drop this
    // reconstruction.
    let witness = evaluate_permutation(state_in);
    let sout_mle = PermMle::from_witness(&witness).sout;
    let v0 = evaluate_slice(&sout_mle, &r0);
    ...
    let claim3 = derive_state_claim(&proof.sin_r3_check, &rs3, &active_mle, &rc_mle)?;
    let rs4 = verify_product(&proof.sin_r4_check, &r4, sin_at_r4, channel)?;
    let claim4 = derive_state_claim(&proof.sin_r4_check, &rs4, &active_mle, &rc_mle)?;
    let rs5 = verify_product(&proof.sin_r5_check, &r5, sin_at_r5, channel)?;
    let claim5 = derive_state_claim(&proof.sin_r5_check, &rs5, &active_mle, &rc_mle)?;
