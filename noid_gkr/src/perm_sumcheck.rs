...
/// outer boundary batching sumcheck (γ₂).
...
/// Like [`prove_perm`] but takes a pre-computed [`PermMle`]. Callers
/// that already built the layered witness (e.g. the spine / auth
/// prover, which uses the same `state` + `sout` tables to assemble the
/// concatenated boundary MLE) avoid a second `evaluate_permutation` +
/// `PermMle::from_witness` per slot.
...
///
/// γ₂-lift: `v0 = sout_mle(r0)` is a caller-supplied input. For
/// public-input circuits (spine) the wrapper computes it natively from
/// the reconstructed `state_in` and passes it down. For
/// witness-secret circuits (auth) it comes from an outer boundary
/// opening, so the verifier never touches the secret.
///
