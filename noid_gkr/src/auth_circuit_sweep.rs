...
//! Typed topology description of the `SweepAuthCircuit`:
...
//! live input `i ∈ 0..N_SWEEP_AUTH_INPUTS`, the circuit recomputes
//!
...
//!
//! `N_SWEEP_AUTH_SLOTS = 5 * N_SWEEP_AUTH_INPUTS = 20` for the canonical `noid_tx`
//! `MAX_INPUTS = 4`.
//!
//! ## Boundary surface
//!
...
//! `SweepAuthPublicInputs`, is not serialized by the Auth proof, and is
//! never absorbed raw into the Fiat-Shamir channel. The current Auth
//! capsule is intentionally documented as **non-ZK**: it exposes
//! deterministic random-point evaluations / PCS artifacts of the private
//! AuthGKR trace. The supported privacy claim is raw-secret non-disclosure
//! plus one-wayness under the Poseidon2b preimage assumption and the stated
...
/// `SweepAuthCircuit` for `TxShape::Sweep25x2`.
pub const N_SWEEP_AUTH_INPUTS: usize = 25;
...
pub const N_SWEEP_AUTH_SLOTS: usize = N_SLOTS_PER_INPUT * N_SWEEP_AUTH_INPUTS;
...
impl SweepAuthSlotRole {
    /// The input index this slot belongs to.
    #[inline]
    pub fn input_idx(&self) -> usize {
        match self {
            SweepAuthSlotRole::HAddrPermA { input_idx }
            | SweepAuthSlotRole::HAddrPermB { input_idx }
            | SweepAuthSlotRole::HAuthPermA { input_idx }
            | SweepAuthSlotRole::HAuthPermB { input_idx }
            | SweepAuthSlotRole::HAuthPermC { input_idx } => *input_idx as usize,
        }
    }
}
...
pub struct SweepAuthSlotDescriptor {
    /// Instance id in `0..N_SWEEP_AUTH_SLOTS`.
    pub id: usize,
    /// Role of this slot.
    pub role: SweepAuthSlotRole,
...
pub struct SweepAuthPublicInputs {
    /// `tx_body_hash = [hi, lo]`. Pinned on the STARK side via the spine bridge.
    pub tx_body_hash: [Block128; 2],
    /// Claimed `Address[i] = [hi, lo]`.
    pub expected_address: [[Block128; 2]; N_SWEEP_AUTH_INPUTS],
    /// Claimed `AuthTag[i] = [hi, lo]`.
    pub expected_auth_tag: [[Block128; 2]; N_SWEEP_AUTH_INPUTS],
}

impl SweepAuthPublicInputs {
    pub fn zero() -> Self {
        Self {
...
/// receives only `SweepAuthPublicInputs` + a pre-built `SweepAuthProofKillShot`.
///
...
impl SweepAuthInputs {
    /// Extract the public-only subset, discarding `spend_secret`.
    pub fn to_public(&self) -> SweepAuthPublicInputs {
...
impl SweepAuthInputs {
    ...
    pub fn zero() -> Self {
        Self {
            spend_secret: [[Block128::ZERO; 2]; N_SWEEP_AUTH_INPUTS],
            tx_body_hash: [Block128::ZERO; 2],
            expected_address: [[Block128::ZERO; 2]; N_SWEEP_AUTH_INPUTS],
            expected_auth_tag: [[Block128::ZERO; 2]; N_SWEEP_AUTH_INPUTS],
        }
...
/// Custom Debug for SweepAuthInputs: spend_secret is redacted to prevent
/// accidental exposure in logs, panic output, or test diagnostics.
/// Only the public fields are shown.
impl std::fmt::Debug for SweepAuthInputs {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SweepAuthInputs")
...
/// Static topology of the 125-slot sweep auth circuit. Cheap to construct
/// (one allocation, `O(N_SWEEP_AUTH_SLOTS)` work), typically built once per
/// process and reused.
#[derive(Debug, Clone)]
pub struct SweepAuthCircuit {
    pub slots: Vec<SweepAuthSlotDescriptor>,
}

impl SweepAuthCircuit {
    /// Build the full topology for the `Sweep25x2` input capacity.
    pub fn build() -> Self {
        let iv_addr = capacity_iv(TAG_ADDRESS);
        let iv_auth = capacity_iv(TAG_AUTHTAG);

        let mut slots = Vec::with_capacity(N_SWEEP_AUTH_SLOTS);
        for input_idx_usize in 0..N_SWEEP_AUTH_INPUTS {
            let input_idx = input_idx_usize as u8;
            let base = slots.len();

            // HAddrPermA — head.
            slots.push(SweepAuthSlotDescriptor {
                id: base,
                role: SweepAuthSlotRole::HAddrPermA { input_idx },
                capacity_iv: iv_addr,
                is_head: true,
            ...
            slots.push(SweepAuthSlotDescriptor {
                id: base + 1,
                role: SweepAuthSlotRole::HAddrPermB { input_idx },
                capacity_iv: iv_addr,
                is_head: false,
            ...
            slots.push(SweepAuthSlotDescriptor {
                id: base + 2,
                role: SweepAuthSlotRole::HAuthPermA { input_idx },
                capacity_iv: iv_auth,
                is_head: true,
                prev_output_src: None,
            });
            ...
            slots.push(SweepAuthSlotDescriptor {
                id: base + 3,
                role: SweepAuthSlotRole::HAuthPermB { input_idx },
                capacity_iv: iv_auth,
                is_head: false,
            ...
            slots.push(SweepAuthSlotDescriptor {
                id: base + 4,
                role: SweepAuthSlotRole::HAuthPermC { input_idx },
                capacity_iv: iv_auth,
                is_head: false,
        ...
        debug_assert_eq!(slots.len(), N_SWEEP_AUTH_SLOTS);
        Self { slots }
    }
    ...
    pub fn haddr_output_slot(input_idx: usize) -> usize {
        debug_assert!(input_idx < N_SWEEP_AUTH_INPUTS);
...
mod tests {
    ...
    fn build_has_expected_slot_count_and_shape() {
        let c = SweepAuthCircuit::build();
        assert_eq!(c.slots.len(), N_SWEEP_AUTH_SLOTS);
        assert_eq!(N_SWEEP_AUTH_SLOTS, 5 * N_SWEEP_AUTH_INPUTS);
    }
    ...
    fn heads_are_ivs_non_heads_chain_prev() {
        ...
        for s in &c.slots {
            match s.role {
                SweepAuthSlotRole::HAddrPermA { .. } | SweepAuthSlotRole::HAuthPermA { .. } => {
                    assert!(s.is_head);
                    assert!(s.prev_output_src.is_none());
                }
                SweepAuthSlotRole::HAddrPermB { .. }
                | SweepAuthSlotRole::HAuthPermB { .. }
    ...
    fn ivs_match_domain_tags() {
        ...
        for s in &c.slots {
            match s.role {
                SweepAuthSlotRole::HAddrPermA { .. } | SweepAuthSlotRole::HAddrPermB { .. } => {
                    assert_eq!(s.capacity_iv, iv_addr);
                }
                SweepAuthSlotRole::HAuthPermA { .. }
                | SweepAuthSlotRole::HAuthPermB { .. }
                | SweepAuthSlotRole::HAuthPermC { .. } => {
    ...
    fn output_slot_accessors_land_on_correct_role() {
        ...
        for i in 0..N_SWEEP_AUTH_INPUTS {
            let addr_slot = &c.slots[SweepAuthCircuit::haddr_output_slot(i)];
            assert!(
                matches!(addr_slot.role, SweepAuthSlotRole::HAddrPermB { input_idx } if input_idx as usize == i)
            );

            let auth_slot = &c.slots[SweepAuthCircuit::hauth_output_slot(i)];
            assert!(
                matches!(auth_slot.role, SweepAuthSlotRole::HAuthPermC { input_idx } if input_idx as usize == i)
            );
        }
    }
}
