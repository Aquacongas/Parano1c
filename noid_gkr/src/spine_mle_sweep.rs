...
//! Unified 17-variable MLE layout for the Sweep25x2 Spine Kill-Shot.
//!
//! Architecture
//! ------------
//!
//! The kill-shot prover represents the full 142-permutation sweep spine as
//! four side-by-side 17-variable MLEs on a single hypercube of
//! `2^17 = 131 072` cells:
//!
...
//!
//! Bit layout (17 bits, low → high): `elem:2 | round:7 | slot:8`.
//! `(slot, round, elem)` lives at index
//! `(slot << (N_SWEEP_SPINE_ROUND_VARS + N_SWEEP_SPINE_ELEM_VARS))
//!  | (round << N_SWEEP_SPINE_ELEM_VARS)
//!  | elem`.
//!
...
//!   - Slots `N_SWEEP_SPINE_SLOTS..256` are zero-padded.
//!
...
//! kills the constraint). Padded cells (slot ≥ N_SWEEP_SPINE_SLOTS or
...
//! `μ(dec(y)) = 1[slot(dec(y)) < N_SWEEP_SPINE_SLOTS ∧ round(dec(y)) < N_ROUNDS]`.
...
/// `log2(256) = 8` slot variables for the 142-slot sweep spine.
pub const N_SWEEP_SPINE_SLOT_VARS: usize = 8;
/// `log2(128) = 7` round variables (covers `N_ROUNDS = 66` with padding).
pub const N_SWEEP_SPINE_ROUND_VARS: usize = 7;
/// `log2(4) = 2` element-within-state variables.
pub const N_SWEEP_SPINE_ELEM_VARS: usize = 2;
/// Total variable count of the unified MLE.
pub const N_SWEEP_SPINE_UNIFIED_VARS: usize =
    N_SWEEP_SPINE_SLOT_VARS + N_SWEEP_SPINE_ROUND_VARS + N_SWEEP_SPINE_ELEM_VARS;
/// `2^17 = 131 072` cells.
pub const N_SWEEP_SPINE_UNIFIED_CELLS: usize = 1 << N_SWEEP_SPINE_UNIFIED_VARS;

/// Padding helpers — compile-time assertions on topology.
const _: () = assert!(N_ROUNDS == 66);
const _: () = assert!(STATE_SIZE == 4);
const _: () = assert!(N_SWEEP_SPINE_SLOTS <= (1 << N_SWEEP_SPINE_SLOT_VARS));
const _: () = assert!(N_ROUNDS <= (1 << N_SWEEP_SPINE_ROUND_VARS));
const _: () = assert!(STATE_SIZE <= (1 << N_SWEEP_SPINE_ELEM_VARS));

/// The four columns of the unified MLE. Each is a length-`2^17`
/// vector of `Block128`. The verifier opens all of them at points
/// derived from the unified sumcheck's final challenge.
#[derive(Debug, Clone)]
pub struct SweepSpineUnifiedMle {
...
impl SweepSpineUnifiedMle {
    /// Empty (all-zero) MLE — useful as a starting buffer before
    /// `populate_slot`.
    pub fn zero() -> Self {
        Self {
            s_in: vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS],
            s_out: vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS],
            sigma: vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS],
            state: vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS],
        }
    }
    ...
    pub fn index(slot: usize, round: usize, elem: usize) -> usize {
        debug_assert!(slot < 1 << N_SWEEP_SPINE_SLOT_VARS);
        debug_assert!(round < 1 << N_SWEEP_SPINE_ROUND_VARS);
        debug_assert!(elem < 1 << N_SWEEP_SPINE_ELEM_VARS);
        (slot << (N_SWEEP_SPINE_ROUND_VARS + N_SWEEP_SPINE_ELEM_VARS))
            | (round << N_SWEEP_SPINE_ELEM_VARS)
            | elem
    }
    ...
    /// `slot` must satisfy `slot < N_SWEEP_SPINE_SLOTS`.
    pub fn populate_slot(&mut self, slot: usize, witness: &PermLayerWitness) {
        assert!(slot < N_SWEEP_SPINE_SLOTS, "slot out of range");
    ...
    pub fn debug_check_identity(&self) {
        use crate::spine_shift_sweep::{
            build_sweep_spine_mu_table, build_sweep_spine_rc_table, sweep_spine_dec_round_index,
            sweep_spine_mds_coeff, sweep_spine_round_of,
        };
        use noid_core::packed::pow7::pow7_block128;

        let rc = build_sweep_spine_rc_table();
        ...
        for idx_y in 0..N_SWEEP_SPINE_UNIFIED_CELLS {
            let dec = sweep_spine_dec_round_index(idx_y as u32) as usize;
            if mu[dec] == Block128::ZERO {
                continue;
            }
            let r = sweep_spine_round_of(dec as u32);
            ...
            for j in 0..STATE_SIZE {
                acc += sweep_spine_mds_coeff(r, e, j) * pi[j];
            }
...
/// Build the unified MLE from `N_SWEEP_SPINE_SLOTS` `state_in` vectors. The
...
pub fn build_sweep_spine_unified_mle(
    slot_state_ins: &[[Block128; STATE_SIZE]],
) -> (SweepSpineUnifiedMle, Vec<PermLayerWitness>) {
    assert_eq!(
        slot_state_ins.len(),
        N_SWEEP_SPINE_SLOTS,
        "expected exactly N_SWEEP_SPINE_SLOTS slot inputs"
    );
    let mut mle = SweepSpineUnifiedMle::zero();
    let mut witnesses = Vec::with_capacity(N_SWEEP_SPINE_SLOTS);
...
pub fn sweep_spine_sigma_at(round: usize, elem: usize) -> Block128 {
...
mod tests {
    ...
    fn topology_constants_consistent() {
        assert_eq!(N_SWEEP_SPINE_UNIFIED_VARS, 17);
        assert_eq!(N_SWEEP_SPINE_UNIFIED_CELLS, 131_072);
    }
    ...
    fn index_round_trip() {
        ...
        for slot in 0..N_SWEEP_SPINE_SLOTS {
            for round in 0..N_ROUNDS {
                for elem in 0..STATE_SIZE {
                    let idx = SweepSpineUnifiedMle::index(slot, round, elem);
                    assert!(idx < N_SWEEP_SPINE_UNIFIED_CELLS);
    ...
    fn sigma_zero_outside_topology() {
        assert_eq!(sweep_spine_sigma_at(N_ROUNDS, 0), Block128::ZERO);
        assert_eq!(sweep_spine_sigma_at(N_ROUNDS + 1, 0), Block128::ZERO);
        assert_eq!(sweep_spine_sigma_at(0, STATE_SIZE), Block128::ZERO);
    }
    ...
    fn populate_slot_matches_native_permutation() {
        ...
        for round in 0..N_ROUNDS {
            for elem in 0..STATE_SIZE {
                let idx = SweepSpineUnifiedMle::index(7, round, elem);
                assert_eq!(mle.sigma[idx], sweep_spine_sigma_at(round, elem));
            }
        }
    }
    ...
    fn padded_cells_are_zero() {
        ...
        for slot in N_SWEEP_SPINE_SLOTS..(1 << N_SWEEP_SPINE_SLOT_VARS) {
            for round in 0..(1 << N_SWEEP_SPINE_ROUND_VARS) {
                for elem in 0..(1 << N_SWEEP_SPINE_ELEM_VARS) {
                    let idx = SweepSpineUnifiedMle::index(slot, round, elem);
        ...
        for slot in 0..N_SWEEP_SPINE_SLOTS {
            for round in N_ROUNDS..(1 << N_SWEEP_SPINE_ROUND_VARS) {
                for elem in 0..STATE_SIZE {
                    let idx = SweepSpineUnifiedMle::index(slot, round, elem);
                    assert_eq!(mle.s_in[idx], Block128::ZERO);
                    assert_eq!(mle.s_out[idx], Block128::ZERO);
