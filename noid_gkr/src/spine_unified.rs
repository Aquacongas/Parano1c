...
mod tests {
    ...
    fn flat_round_poly_matches_tower_round_poly() {
        ...
        for round in 0..N_SPINE_UNIFIED_VARS {
            ...
            let p_flt_eval =
                compute_round_polynomial_flat_per_eval(&flt, beta_flat, gamma_flat);
            assert_eq!(
    ...
    fn flat_shift_round_poly_matches_tower() {
        ...
        let p_tow = compute_shift_round_polynomial(
            &s_in, &s_out, &state, &w_sin, &w_sout, &w_state,
        );
    ...
    fn shift_gadget_verifies_end_to_end() {
        ...
        let shift_red = verify_spine_shift(&proof.shift, &main_red, &mut ch)
            .expect("shift verify");
