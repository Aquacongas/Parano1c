...
//! Sweep AuthGKR unified Kill-Shot sumcheck driver, retargeted on
//! the 16-variable hypercube.
//!
//! Forked from `spine_unified.rs` (main sumcheck / shift gadget / flat-basis /
//! monomial-form variants). The protocol shape is identical:
//!
//!   * 16-round main sumcheck of degree 9 (C1 + β·C1' + γ·C2)
//!   * 16-round shift gadget of degree 2 (reduces 11 `_dec` claims to
//!     three direct openings on the original `s_in`, `s_out`, `state`
//!     columns)
//!
//! Differences from Spine:
//!
//!   * `N_SWEEP_AUTH_UNIFIED_VARS = 16`.
//!   * Slot field width 7 bits; cell count `2^16 = 65K`.
//!   * Public schedules pulled from cached `auth_shift_sweep::sweep_auth_*_table()`
//!     getters — no per-proof rebuild.
//!   * Live-slot count fixed at `N_SWEEP_AUTH_LIVE_SLOTS = 125`.
...
use crate::auth_mle_sweep::{
    SweepAuthUnifiedMle, N_SWEEP_AUTH_ELEM_VARS, N_SWEEP_AUTH_ROUND_VARS, N_SWEEP_AUTH_SLOT_BITS,
    N_SWEEP_AUTH_UNIFIED_CELLS, N_SWEEP_AUTH_UNIFIED_VARS,
};
use crate::auth_shift_sweep::{
    sweep_auth_build_u_table, sweep_auth_mds_lane_tables, sweep_auth_mds_lane_tables_flat,
    sweep_auth_permute_by_dec, sweep_auth_project_lane, sweep_auth_rc_dec_table_flat,
    sweep_auth_rc_table, sweep_auth_sigma_dec_lane_tables_flat, sweep_auth_sigma_dec_table_flat,
    sweep_auth_sigma_table,
};

const ELEM_LO: usize = 0;
const ELEM_HI: usize = ELEM_LO + N_SWEEP_AUTH_ELEM_VARS;
const ROUND_LO: usize = ELEM_HI;
const ROUND_HI: usize = ROUND_LO + N_SWEEP_AUTH_ROUND_VARS;
const SLOT_LO: usize = ROUND_HI;
const SLOT_HI: usize = SLOT_LO + N_SWEEP_AUTH_SLOT_BITS;

/// Per-variable degree of the main round polynomial.
pub const SWEEP_AUTH_UNIFIED_ROUND_DEGREE: usize = 9;

/// Per-variable degree of the shift-gadget round polynomial.
pub const SWEEP_AUTH_SHIFT_ROUND_DEGREE: usize = 2;

/// Number of witness-derived claims emitted by the main sumcheck.
pub const N_SWEEP_AUTH_UNIFIED_WITNESS_CLAIMS: usize = 12;

#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct SweepAuthUnifiedProof {
...
pub struct SweepAuthUnifiedReduction {
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct SweepAuthShiftProof {
    pub round_polys: Vec<RoundPolynomial<Block128>>,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SweepAuthShiftReduction {
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct SweepAuthKillShotProof {
    pub main: SweepAuthUnifiedProof,
    pub shift: SweepAuthShiftProof,
}
...
pub fn prove_sweep_auth_unified<T: FiatShamir<Block128>>(
    mle: &SweepAuthUnifiedMle,
    channel: &mut T,
) -> (SweepAuthUnifiedProof, Vec<Block128>) {
    assert_eq!(mle.s_in.len(), N_SWEEP_AUTH_UNIFIED_CELLS);
    assert_eq!(mle.s_out.len(), N_SWEEP_AUTH_UNIFIED_CELLS);
    assert_eq!(mle.sigma.len(), N_SWEEP_AUTH_UNIFIED_CELLS);
    assert_eq!(mle.state.len(), N_SWEEP_AUTH_UNIFIED_CELLS);

    let rho: Vec<Block128> = (0..N_SWEEP_AUTH_UNIFIED_VARS)
    ...
    let mut round_polys = Vec::with_capacity(N_SWEEP_AUTH_UNIFIED_VARS);
    let mut r_prime = vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_VARS];

    for round in 0..N_SWEEP_AUTH_UNIFIED_VARS {
        ...
        r_prime[N_SWEEP_AUTH_UNIFIED_VARS - 1 - round] = challenge;
    ...
    let proof = SweepAuthUnifiedProof {
        round_polys,
...
pub fn verify_sweep_auth_unified<T: FiatShamir<Block128>>(
    proof: &SweepAuthUnifiedProof,
    channel: &mut T,
) -> Option<SweepAuthUnifiedReduction> {
    if proof.round_polys.len() != N_SWEEP_AUTH_UNIFIED_VARS {
        return None;
    }
    for p in &proof.round_polys {
        if p.degree() > SWEEP_AUTH_UNIFIED_ROUND_DEGREE {
            return None;
        }
    }
    ...
    let u_at_r = evaluate_flat(&sweep_auth_build_u_table(&rho), &r_prime);
    let sigma_dec_at_r = evaluate_preflat(sweep_auth_sigma_dec_table_flat(), &r_prime);
    let rc_dec_at_r = evaluate_preflat(sweep_auth_rc_dec_table_flat(), &r_prime);
    let mut mds_lane_dec_at_r = [Block128::ZERO; STATE_SIZE];
    let mut sigma_lane_dec_at_r = [Block128::ZERO; STATE_SIZE];
    let mds_lane_flat = sweep_auth_mds_lane_tables_flat();
    let sigma_lane_flat = sweep_auth_sigma_dec_lane_tables_flat();
    ...
    Some(SweepAuthUnifiedReduction {
        r_prime,
...
fn build_unified_tables(mle: &SweepAuthUnifiedMle, rho: &[Block128]) -> UnifiedTables {
    let sigma_full = sweep_auth_sigma_table().to_vec();
    let sigma_dec = sweep_auth_permute_by_dec(&sigma_full);
    let rc_dec = sweep_auth_permute_by_dec(sweep_auth_rc_table());
    let s_in_dec = sweep_auth_permute_by_dec(&mle.s_in);
    let s_out_dec = sweep_auth_permute_by_dec(&mle.s_out);
    let state_dec = sweep_auth_permute_by_dec(&mle.state);

    let mds_lane_dec: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|j| sweep_auth_mds_lane_tables()[j].clone());
    ...
    let s_out_lane_dec: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|j| sweep_auth_project_lane(&s_out_dec, j));
    let state_lane_dec: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|j| sweep_auth_project_lane(&state_dec, j));

    UnifiedTables {
        u: sweep_auth_build_u_table(rho),
        sigma_dec,
        rc_dec,
        mds_lane_dec,
        sigma_lane_dec,
...
fn compute_round_polynomial_flat(
    ...
    // For auth (N_SWEEP_AUTH_UNIFIED_CELLS=16384, half=8192 initially), rayon
    // overhead (~100μs) is comparable to compute, so use threshold 4096.
    let acc: [u128; SWEEP_AUTH_UNIFIED_ROUND_DEGREE + 1] = if half >= 4096 {
        (0..half)
            .into_par_iter()
            .fold(
                || [0u128; SWEEP_AUTH_UNIFIED_ROUND_DEGREE + 1],
                |mut local_acc, i| {
                    ...
                    for k in 0..=SWEEP_AUTH_UNIFIED_ROUND_DEGREE {
                        local_acc[k] ^= f_p[k];
                    }
                    local_acc
                },
        ...
        let mut acc = [0u128; SWEEP_AUTH_UNIFIED_ROUND_DEGREE + 1];
        for i in 0..half {
            let u_p: [u128; 2] = [tabs.u[i], tabs.u[i] ^ tabs.u[i + half]];
            let sg_p: [u128; 2] = [
                tabs.sigma_dec[i],
...
pub fn prove_sweep_auth_shift<T: FiatShamir<Block128>>(
    mle: &SweepAuthUnifiedMle,
    main_red_r_prime: &[Block128],
    channel: &mut T,
) -> (SweepAuthShiftProof, Vec<Block128>) {
    assert_eq!(main_red_r_prime.len(), N_SWEEP_AUTH_UNIFIED_VARS);
    ...
    let mut round_polys = Vec::with_capacity(N_SWEEP_AUTH_UNIFIED_VARS);
    let mut r_double_prime = vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_VARS];
    for round in 0..N_SWEEP_AUTH_UNIFIED_VARS {
        let poly =
        ...
        r_double_prime[N_SWEEP_AUTH_UNIFIED_VARS - 1 - round] = r;
        round_polys.push(poly);
    }
...
pub fn verify_sweep_auth_shift<T: FiatShamir<Block128>>(
    proof: &SweepAuthShiftProof,
    main_red: &SweepAuthUnifiedReduction,
    channel: &mut T,
) -> Option<SweepAuthShiftReduction> {
    if proof.round_polys.len() != N_SWEEP_AUTH_UNIFIED_VARS {
        return None;
    }
    for p in &proof.round_polys {
        if p.degree() > SWEEP_AUTH_SHIFT_ROUND_DEGREE {
            return None;
        }
    }
    let delta = channel.squeeze();
...
fn build_combined_weights(r_prime: &[Block128], delta: Block128) -> CombinedWeights {
    ...
    let n_rounds = 1usize << N_SWEEP_AUTH_ROUND_VARS;
    ...
    let mut w_sin = vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_CELLS];
    let mut w_sout = vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_CELLS];
    let mut w_state = vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_CELLS];

    for x in 0..N_SWEEP_AUTH_UNIFIED_CELLS {
        let slot = (x >> SLOT_LO) & ((1 << N_SWEEP_AUTH_SLOT_BITS) - 1);
        let round = (x >> ROUND_LO) & ((1 << N_SWEEP_AUTH_ROUND_VARS) - 1);
        let elem = x & ((1 << N_SWEEP_AUTH_ELEM_VARS) - 1);
...
fn combined_target(red: &SweepAuthUnifiedReduction, delta: Block128) -> Block128 {
    let d0 = Block128::ONE;
    let d1 = delta;
    let d2 = d1 * delta;
...
fn compute_shift_round_polynomial_flat(
    ...
    let half = s_in.len() / 2;
    let mut evals = [0u128; SWEEP_AUTH_SHIFT_ROUND_DEGREE + 1];
    let t_flat: [u128; SWEEP_AUTH_SHIFT_ROUND_DEGREE + 1] =
    ...
    for i in 0..half {
        ...
        for k in 0..=SWEEP_AUTH_SHIFT_ROUND_DEGREE {
            let t = t_flat[k];
    ...
    let evals_tower: [Block128; SWEEP_AUTH_SHIFT_ROUND_DEGREE + 1] =
...
mod tests {
    use super::*;
    use crate::auth_mle_sweep::{build_sweep_auth_unified_mle, N_SWEEP_AUTH_LIVE_SLOTS};
    ...
    fn random_mle(seed: u128) -> SweepAuthUnifiedMle {
        let state_ins: Vec<_> = (0..N_SWEEP_AUTH_LIVE_SLOTS)
    ...
    fn round_degree_constants() {
        assert_eq!(SWEEP_AUTH_UNIFIED_ROUND_DEGREE, 9);
        assert_eq!(SWEEP_AUTH_SHIFT_ROUND_DEGREE, 2);
    }
    ...
    fn honest_main_verifies() {
        ...
        let (proof, _r_prime) = prove_sweep_auth_unified(&mle, &mut ch_p);
        assert_eq!(proof.round_polys.len(), N_SWEEP_AUTH_UNIFIED_VARS);

        let mut ch_v = Poseidon2bChannel::new();
        let red = verify_sweep_auth_unified(&proof, &mut ch_v).expect("verify must accept");
        assert_eq!(red.r_prime.len(), N_SWEEP_AUTH_UNIFIED_VARS);
    ...
    fn final_main_claims_match_native_evaluations() {
        ...
        let mut ch_v = Poseidon2bChannel::new();
        let red = verify_sweep_auth_unified(&proof, &mut ch_v).unwrap();

        assert_eq!(evaluate_slice(&mle.state, &red.r_prime), red.state_at_r);
        assert_eq!(
            evaluate_slice(&sweep_auth_permute_by_dec(&mle.s_in), &red.r_prime),
            red.s_in_dec_at_r
        );
        assert_eq!(
            evaluate_slice(&sweep_auth_permute_by_dec(&mle.s_out), &red.r_prime),
            red.s_out_dec_at_r
        );
        assert_eq!(
            evaluate_slice(&sweep_auth_permute_by_dec(&mle.state), &red.r_prime),
            red.state_dec_at_r
        );

        let s_out_dec_full = sweep_auth_permute_by_dec(&mle.s_out);
        let state_dec_full = sweep_auth_permute_by_dec(&mle.state);
        for j in 0..STATE_SIZE {
            assert_eq!(
                evaluate_slice(&sweep_auth_project_lane(&s_out_dec_full, j), &red.r_prime),
                red.s_out_lane_dec_at_r[j],
                "s_out lane {j}"
            );
            assert_eq!(
                evaluate_slice(&sweep_auth_project_lane(&state_dec_full, j), &red.r_prime),
    ...
    fn tampered_state_claim_is_rejected() {
        ...
        let mut ch_p = Poseidon2bChannel::new();
        let (mut proof, _r_prime) = prove_sweep_auth_unified(&mle, &mut ch_p);
        proof.state_at_r += Block128::ONE;

        let mut ch_v = Poseidon2bChannel::new();
        assert!(verify_sweep_auth_unified(&proof, &mut ch_v).is_none());
    }
    ...
    fn run_full_pipeline(seed: u128) -> (SweepAuthUnifiedMle, SweepAuthKillShotProof) {
        let mle = random_mle(seed);
        let mut ch = Poseidon2bChannel::new();
        let (main, r_prime) = prove_sweep_auth_unified(&mle, &mut ch);
        let (shift, _r2) = prove_sweep_auth_shift(&mle, &r_prime, &mut ch);
        (mle, SweepAuthKillShotProof { main, shift })
    }

    #[test]
    fn shift_gadget_round_count_and_degree() {
        let (_, proof) = run_full_pipeline(101);
        assert_eq!(proof.shift.round_polys.len(), N_SWEEP_AUTH_UNIFIED_VARS);
        for p in &proof.shift.round_polys {
            assert!(p.degree() <= SWEEP_AUTH_SHIFT_ROUND_DEGREE);
        }
    }
    ...
    fn shift_gadget_verifies_end_to_end() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_sweep_auth_unified(&proof.main, &mut ch).expect("main verify");
        let shift_red =
            verify_sweep_auth_shift(&proof.shift, &main_red, &mut ch).expect("shift verify");
        assert_eq!(shift_red.r_double_prime.len(), N_SWEEP_AUTH_UNIFIED_VARS);
    }
    ...
    fn shift_final_claims_match_native_evaluations() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_sweep_auth_unified(&proof.main, &mut ch).unwrap();
        let shift_red = verify_sweep_auth_shift(&proof.shift, &main_red, &mut ch).unwrap();
    ...
    fn shift_tampered_final_claim_is_rejected() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_sweep_auth_unified(&proof.main, &mut ch).unwrap();
        assert!(verify_sweep_auth_shift(&proof.shift, &main_red, &mut ch).is_none());
    }
    ...
    fn shift_combined_target_matches_first_round_sum() {
        ...
        let mut ch_v = Poseidon2bChannel::new();
        let main_red = verify_sweep_auth_unified(&main, &mut ch_v).unwrap();
