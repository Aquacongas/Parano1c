...
//! Sweep AuthGKR dedicated 16-variable unified MLE.
//!
//! Architecture
//! ------------
//!
//! Sweep AuthGKR drives `N_SWEEP_AUTH_LIVE_SLOTS = 125` Poseidon2b permutations
//! (25 inputs × 5 perms each: HAddrPermA, HAddrPermB, HAuthPermA,
//! HAuthPermB, HAuthPermC). The live slots fit in a 7-bit slot field,
//! leaving 3 padded slots in the 128-slot hypercube:
//!
//! Bit layout (16 bits, low → high): `elem:2 | round:7 | slot:7`.
//! `(slot, round, elem)` lives at index
//! `(slot << 9) | (round << 2) | elem`.
//!
//! The elem/round bit positions match the standard AuthGKR path; only the
//! slot field widens from 5 bits to 7. Every per-cell identity (C1, C1', C2)
//! remains identical.
//!
...
//!   - Slots `N_SWEEP_AUTH_LIVE_SLOTS..128` are zero-padded.
...
/// Number of live Sweep AuthGKR permutation slots: 25 inputs × 5 perms.
pub const N_SWEEP_AUTH_LIVE_SLOTS: usize = 125;

/// `log2(128) = 7` slot variables.
pub const N_SWEEP_AUTH_SLOT_BITS: usize = 7;
/// `log2(128) = 7` round variables (covers `N_ROUNDS = 66` with padding).
pub const N_SWEEP_AUTH_ROUND_VARS: usize = 7;
/// `log2(4) = 2` element-within-state variables.
pub const N_SWEEP_AUTH_ELEM_VARS: usize = 2;
/// Total variable count of the AuthGKR unified MLE.
pub const N_SWEEP_AUTH_UNIFIED_VARS: usize =
    N_SWEEP_AUTH_SLOT_BITS + N_SWEEP_AUTH_ROUND_VARS + N_SWEEP_AUTH_ELEM_VARS;
/// `2^16 = 65 536` cells.
pub const N_SWEEP_AUTH_UNIFIED_CELLS: usize = 1 << N_SWEEP_AUTH_UNIFIED_VARS;

const _: () = assert!(N_ROUNDS == 66);
const _: () = assert!(STATE_SIZE == 4);
const _: () = assert!(N_SWEEP_AUTH_LIVE_SLOTS <= (1 << N_SWEEP_AUTH_SLOT_BITS));
const _: () = assert!(N_ROUNDS <= (1 << N_SWEEP_AUTH_ROUND_VARS));
const _: () = assert!(STATE_SIZE <= (1 << N_SWEEP_AUTH_ELEM_VARS));
...
impl SweepAuthUnifiedMle {
    /// Empty (all-zero) MLE — useful as a starting buffer before
    /// `populate_slot`.
    pub fn zero() -> Self {
        Self {
            s_in: vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_CELLS],
            s_out: vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_CELLS],
            sigma: vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_CELLS],
            state: vec![Block128::ZERO; N_SWEEP_AUTH_UNIFIED_CELLS],
        }
    }

    /// Index into the AuthGKR unified MLE for `(slot, round, elem)`.
    /// Bit positions for elem/round match standard AuthGKR; only the slot
    /// field widens from 5 to 7 bits.
    #[inline]
    pub fn index(slot: usize, round: usize, elem: usize) -> usize {
        debug_assert!(slot < 1 << N_SWEEP_AUTH_SLOT_BITS);
        debug_assert!(round < 1 << N_SWEEP_AUTH_ROUND_VARS);
        debug_assert!(elem < 1 << N_SWEEP_AUTH_ELEM_VARS);
        (slot << (N_SWEEP_AUTH_ROUND_VARS + N_SWEEP_AUTH_ELEM_VARS))
            | (round << N_SWEEP_AUTH_ELEM_VARS)
            | elem
    }

    /// Fill in one slot's cells from an instrumented `PermLayerWitness`.
    /// `slot` must satisfy `slot < N_SWEEP_AUTH_LIVE_SLOTS`.
    pub fn populate_slot(&mut self, slot: usize, witness: &PermLayerWitness) {
        assert!(slot < N_SWEEP_AUTH_LIVE_SLOTS, "slot out of range");
        for r in 0..N_ROUNDS {
...
pub fn sweep_auth_sigma_at(round: usize, elem: usize) -> Block128 {
...
/// Build the AuthGKR unified MLE from `N_SWEEP_AUTH_LIVE_SLOTS` `state_in`
/// vectors, one per permutation slot in post-order.
pub fn build_sweep_auth_unified_mle(
    slot_state_ins: &[[Block128; STATE_SIZE]],
) -> (SweepAuthUnifiedMle, Vec<PermLayerWitness>) {
    assert_eq!(
        slot_state_ins.len(),
        N_SWEEP_AUTH_LIVE_SLOTS,
        "expected exactly N_SWEEP_AUTH_LIVE_SLOTS slot inputs"
    );
    let mut mle = SweepAuthUnifiedMle::zero();
    let mut witnesses = Vec::with_capacity(N_SWEEP_AUTH_LIVE_SLOTS);
...
mod tests {
    ...
    fn topology_constants_consistent() {
        assert_eq!(N_SWEEP_AUTH_UNIFIED_VARS, 16);
        assert_eq!(N_SWEEP_AUTH_UNIFIED_CELLS, 65_536);
    }
    ...
    fn index_round_trip() {
        for slot in 0..N_SWEEP_AUTH_LIVE_SLOTS {
            for round in 0..N_ROUNDS {
                for elem in 0..STATE_SIZE {
                    let idx = SweepAuthUnifiedMle::index(slot, round, elem);
                    assert!(idx < N_SWEEP_AUTH_UNIFIED_CELLS);
    ...
    #[test]
    fn build_sweep_auth_unified_mle_round_trips() {
        let state_ins: Vec<_> = (0..N_SWEEP_AUTH_LIVE_SLOTS)
            .map(|i| random_state(i as u128 + 1))
            .collect();
        let (mle, witnesses) = build_sweep_auth_unified_mle(&state_ins);
        assert_eq!(witnesses.len(), N_SWEEP_AUTH_LIVE_SLOTS);
        ...
        for slot in N_SWEEP_AUTH_LIVE_SLOTS..(1 << N_SWEEP_AUTH_SLOT_BITS) {
            for round in 0..(1 << N_SWEEP_AUTH_ROUND_VARS) {
                for elem in 0..(1 << N_SWEEP_AUTH_ELEM_VARS) {
                    let idx = SweepAuthUnifiedMle::index(slot, round, elem);
        ...
        for slot in 0..N_SWEEP_AUTH_LIVE_SLOTS {
            for round in N_ROUNDS..(1 << N_SWEEP_AUTH_ROUND_VARS) {
                for elem in 0..STATE_SIZE {
                    let idx = SweepAuthUnifiedMle::index(slot, round, elem);
                    assert_eq!(mle.s_in[idx], Block128::ZERO);
                    assert_eq!(mle.s_out[idx], Block128::ZERO);
