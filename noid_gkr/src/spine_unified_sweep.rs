...
//!
//! Discharges the full sweep Spine algebraic surface (C1 + β·C1' + γ·C2)
//! in a single degree-9 sumcheck of length `N_SWEEP_SPINE_UNIFIED_VARS = 17`.
//!
...
//!
//! After the 17 main rounds, the prover holds 12 witness-derived
//! claims at the final point `r' ∈ F^17`:
//!
...
//! using `evaluate_slice` — `O(2^17)` per evaluation (precomputed
//! tables, no FFT).
...
use crate::spine_mle_sweep::{
    SweepSpineUnifiedMle, N_SWEEP_SPINE_ELEM_VARS, N_SWEEP_SPINE_ROUND_VARS,
    N_SWEEP_SPINE_SLOT_VARS, N_SWEEP_SPINE_UNIFIED_CELLS, N_SWEEP_SPINE_UNIFIED_VARS,
};
use crate::spine_shift_sweep::{
    build_sweep_spine_mds_lane_table_for_live_slots, build_sweep_spine_rc_table_for_live_slots,
    build_sweep_spine_sigma_table_for_live_slots, build_sweep_spine_u_table_for_live_slots,
    sweep_spine_permute_by_dec, sweep_spine_project_lane, sweep_sweep_spine_mds_lane_tables_flat,
    sweep_sweep_spine_rc_dec_table_flat, sweep_sweep_spine_sigma_dec_lane_tables_flat,
    sweep_sweep_spine_sigma_dec_table_flat,
};

// Bit layout: `elem:2 | round:7 | slot:8` (low → high). Slices over
// `r_prime[bit_idx]` follow the same convention as `evaluate_slice`.
const ELEM_LO: usize = 0;
const ELEM_HI: usize = ELEM_LO + N_SWEEP_SPINE_ELEM_VARS;
const ROUND_LO: usize = ELEM_HI;
const ROUND_HI: usize = ROUND_LO + N_SWEEP_SPINE_ROUND_VARS;
const SLOT_LO: usize = ROUND_HI;
const SLOT_HI: usize = SLOT_LO + N_SWEEP_SPINE_SLOT_VARS;
...
pub const SWEEP_SPINE_UNIFIED_ROUND_DEGREE: usize = 9;

/// Number of witness-derived claims emitted by the main sumcheck.
/// See module docstring for the catalog.
pub const N_SWEEP_UNIFIED_WITNESS_CLAIMS: usize = 12;

/// Output of the main unified sumcheck prover.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct SweepSpineUnifiedProof {
    /// `N_SWEEP_SPINE_UNIFIED_VARS` round polynomials, each of degree at
    /// most `SWEEP_SPINE_UNIFIED_ROUND_DEGREE`.
...
pub struct SweepSpineUnifiedReduction {
...
/// Run the unified main sumcheck on a populated `SweepSpineUnifiedMle`.
///
...
pub fn prove_sweep_spine_unified<T: FiatShamir<Block128>>(
    mle: &SweepSpineUnifiedMle,
    channel: &mut T,
) -> (SweepSpineUnifiedProof, Vec<Block128>) {
    prove_sweep_spine_unified_for_live_slots(mle, N_SWEEP_SPINE_SLOTS, channel)
}

/// Live-slot-parameterised variant for AuthGKR (`live_slots = 20`)
/// and any other topology that reuses the unified hypercube.
pub fn prove_sweep_spine_unified_for_live_slots<T: FiatShamir<Block128>>(
    mle: &SweepSpineUnifiedMle,
    live_slots: usize,
    channel: &mut T,
) -> (SweepSpineUnifiedProof, Vec<Block128>) {
    assert_eq!(mle.s_in.len(), N_SWEEP_SPINE_UNIFIED_CELLS);
    assert_eq!(mle.s_out.len(), N_SWEEP_SPINE_UNIFIED_CELLS);
    assert_eq!(mle.sigma.len(), N_SWEEP_SPINE_UNIFIED_CELLS);
    assert_eq!(mle.state.len(), N_SWEEP_SPINE_UNIFIED_CELLS);

    let rho: Vec<Block128> = (0..N_SWEEP_SPINE_UNIFIED_VARS)
    ...
    let mut round_polys = Vec::with_capacity(N_SWEEP_SPINE_UNIFIED_VARS);
    let mut r_prime = vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_VARS];

    for round in 0..N_SWEEP_SPINE_UNIFIED_VARS {
        ...
        r_prime[N_SWEEP_SPINE_UNIFIED_VARS - 1 - round] = challenge;
    ...
    let proof = SweepSpineUnifiedProof {
        round_polys,
...
pub fn verify_sweep_spine_unified<T: FiatShamir<Block128>>(
    proof: &SweepSpineUnifiedProof,
    channel: &mut T,
) -> Option<SweepSpineUnifiedReduction> {
    verify_sweep_spine_unified_for_live_slots(proof, N_SWEEP_SPINE_SLOTS, channel)
}

/// Live-slot-parameterised variant of `verify_sweep_spine_unified`. Must be
/// called with the same `live_slots` value the prover used.
pub fn verify_sweep_spine_unified_for_live_slots<T: FiatShamir<Block128>>(
    proof: &SweepSpineUnifiedProof,
    live_slots: usize,
    channel: &mut T,
) -> Option<SweepSpineUnifiedReduction> {
    if proof.round_polys.len() != N_SWEEP_SPINE_UNIFIED_VARS {
        return None;
    }
    for p in &proof.round_polys {
        if p.degree() > SWEEP_SPINE_UNIFIED_ROUND_DEGREE {
            return None;
        }
    }
    ...
    //
    // When live_slots == N_SWEEP_SPINE_SLOTS (production path), we use the `OnceLock`
    // getters from spine_shift. For other values we fall back to building
    // the tables on demand (test / non-standard topologies only).
    let u_at_r = evaluate_flat(
        &build_sweep_spine_u_table_for_live_slots(&rho, live_slots),
        &r_prime,
    );

    let (sigma_dec_at_r, rc_dec_at_r, mds_lane_dec_at_r, sigma_lane_dec_at_r) = if live_slots
        == N_SWEEP_SPINE_SLOTS
    {
        // Hot path: use pre-flat tables to skip per-element tower_to_flat conversion.
        let sigma_dec_at_r = evaluate_preflat(sweep_sweep_spine_sigma_dec_table_flat(), &r_prime);
        let rc_dec_at_r = evaluate_preflat(sweep_sweep_spine_rc_dec_table_flat(), &r_prime);
        let mut mds_lane = [Block128::ZERO; STATE_SIZE];
        let mut sigma_lane = [Block128::ZERO; STATE_SIZE];
        let mds_tables = sweep_sweep_spine_mds_lane_tables_flat();
        let sigma_lane_tables = sweep_sweep_spine_sigma_dec_lane_tables_flat();
        ...
        let sigma_dec_full =
            sweep_spine_permute_by_dec(&build_sweep_spine_sigma_table_for_live_slots(live_slots));
        let sigma_dec_at_r = evaluate_flat(&sigma_dec_full, &r_prime);
        let rc_dec_at_r = evaluate_flat(
            &sweep_spine_permute_by_dec(&build_sweep_spine_rc_table_for_live_slots(live_slots)),
            &r_prime,
        );
        let mut mds_lane = [Block128::ZERO; STATE_SIZE];
        let mut sigma_lane = [Block128::ZERO; STATE_SIZE];
        for j in 0..STATE_SIZE {
            mds_lane[j] = evaluate_flat(
                &build_sweep_spine_mds_lane_table_for_live_slots(j, live_slots),
                &r_prime,
            );
            sigma_lane[j] = evaluate_flat(&sweep_spine_project_lane(&sigma_dec_full, j), &r_prime);
        }
        (sigma_dec_at_r, rc_dec_at_r, mds_lane, sigma_lane)
    };
    ...
    Some(SweepSpineUnifiedReduction {
        r_prime,
...
fn build_unified_tables(
    ...
    let sigma_full = build_sweep_spine_sigma_table_for_live_slots(live_slots);
    let sigma_dec = sweep_spine_permute_by_dec(&sigma_full);
    let rc_dec = sweep_spine_permute_by_dec(&build_sweep_spine_rc_table_for_live_slots(live_slots));
    let s_in_dec = sweep_spine_permute_by_dec(&mle.s_in);
    let s_out_dec = sweep_spine_permute_by_dec(&mle.s_out);
    let state_dec = sweep_spine_permute_by_dec(&mle.state);

    let mds_lane_dec: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|j| build_sweep_spine_mds_lane_table_for_live_slots(j, live_slots));
    ...
    let s_out_lane_dec: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|j| sweep_spine_project_lane(&s_out_dec, j));
    let state_lane_dec: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|j| sweep_spine_project_lane(&state_dec, j));

    UnifiedTables {
        u: build_sweep_spine_u_table_for_live_slots(rho, live_slots),
        sigma_dec,
        rc_dec,
        mds_lane_dec,
        sigma_lane_dec,
...
fn compute_round_polynomial(
    ...
    let mut evals = [Block128::ZERO; SWEEP_SPINE_UNIFIED_ROUND_DEGREE + 1];
...
fn t_flat_table() -> [u128; SWEEP_SPINE_UNIFIED_ROUND_DEGREE + 1] {
...
fn compute_round_polynomial_flat_per_eval(
    ...
    let half = tabs.u.len() / 2;
    let mut evals = [0u128; SWEEP_SPINE_UNIFIED_ROUND_DEGREE + 1];
    ...
    for i in 0..half {
        ...
        for k in 0..=SWEEP_SPINE_UNIFIED_ROUND_DEGREE {
            let t = t_flat[k];
    ...
    let evals_tower: [Block128; SWEEP_SPINE_UNIFIED_ROUND_DEGREE + 1] =
...
fn compute_round_polynomial_flat(
    ...
    let half = tabs.u.len() / 2;
    let mut acc = [0u128; SWEEP_SPINE_UNIFIED_ROUND_DEGREE + 1]; // 10 coeffs in t
    const ONE_FLAT: u128 = 1u128;
...
// `N_SWEEP_SPINE_UNIFIED_VARS = 15` rounds. Eleven witness claims share the
...
//
// After 17 rounds the verifier learns one final point `r'' ∈ F^17`
...
// 128K-table inner products needed): the weights factorise as products
// of `eq` along the variable axes.
// ===========================================================================

/// Per-variable degree of the shift-gadget round polynomial.
pub const SWEEP_SPINE_SHIFT_ROUND_DEGREE: usize = 2;

#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct SweepSpineShiftProof {
    pub round_polys: Vec<RoundPolynomial<Block128>>,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SweepSpineShiftReduction {
...
/// Combined output of `prove_sweep_spine_unified` + shift gadget. The
...
/// claims.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct SweepSpineKillShotProof {
    pub main: SweepSpineUnifiedProof,
    pub shift: SweepSpineShiftProof,
}

/// Run the shift gadget on the eleven `_dec` claims of an already
/// constructed `SweepSpineUnifiedProof`. The MLE is required because the
...
pub fn prove_sweep_spine_shift<T: FiatShamir<Block128>>(
    mle: &SweepSpineUnifiedMle,
    main_red_r_prime: &[Block128],
    channel: &mut T,
) -> (SweepSpineShiftProof, Vec<Block128>) {
    assert_eq!(main_red_r_prime.len(), N_SWEEP_SPINE_UNIFIED_VARS);
    ...
    let mut round_polys = Vec::with_capacity(N_SWEEP_SPINE_UNIFIED_VARS);
    let mut r_double_prime = vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_VARS];
    for round in 0..N_SWEEP_SPINE_UNIFIED_VARS {
        let poly =
        ...
        r_double_prime[N_SWEEP_SPINE_UNIFIED_VARS - 1 - round] = r;
        round_polys.push(poly);
    }
...
pub fn verify_sweep_spine_shift<T: FiatShamir<Block128>>(
    proof: &SweepSpineShiftProof,
    main_red: &SweepSpineUnifiedReduction,
    channel: &mut T,
) -> Option<SweepSpineShiftReduction> {
    if proof.round_polys.len() != N_SWEEP_SPINE_UNIFIED_VARS {
        return None;
    }
    for p in &proof.round_polys {
        if p.degree() > SWEEP_SPINE_SHIFT_ROUND_DEGREE {
            return None;
        }
    }
    let delta = channel.squeeze();
    ...
    // Recompute the combined weights at r'' natively. No 128K-cell
...
/// Returns three length-`2^17` tables aligned with the natural
...
fn build_combined_weights(r_prime: &[Block128], delta: Block128) -> CombinedWeights {
    ...
    // w_dec[x] = eq(r', inc(x)). Since `sweep_spine_inc_round_index` only touches
    // round bits, we factorise as eq_slot · eq_round_at_inc(round) · eq_elem.
    let mut w_dec = vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS];
    // w_lane[j][x] = eq_slot · eq_round_at_inc(round) · 1[elem==j].
    let mut w_lane: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|_| vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS]);

    let n_slots = 1usize << N_SWEEP_SPINE_SLOT_VARS;
    let n_rounds = 1usize << N_SWEEP_SPINE_ROUND_VARS;
    let n_elems = 1usize << N_SWEEP_SPINE_ELEM_VARS;
    ...
    let mut w_sin = vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS];
    let mut w_sout = vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS];
    let mut w_state = vec![Block128::ZERO; N_SWEEP_SPINE_UNIFIED_CELLS];

    for x in 0..N_SWEEP_SPINE_UNIFIED_CELLS {
        let dec = w_dec[x];
        w_sin[x] = d0 * dec;
        w_sout[x] = d1 * dec;
        w_state[x] = d2 * dec;
        let elem = x & ((1 << N_SWEEP_SPINE_ELEM_VARS) - 1);
...
fn combined_target(red: &SweepSpineUnifiedReduction, delta: Block128) -> Block128 {
    let d0 = Block128::ONE;
    let d1 = delta;
    let d2 = d1 * delta;
...
fn compute_shift_round_polynomial(
    ...
    let half = s_in.len() / 2;
    let mut evals = [Block128::ZERO; SWEEP_SPINE_SHIFT_ROUND_DEGREE + 1];
    for i in 0..half {
...
fn compute_shift_round_polynomial_flat(
    ...
    let half = s_in.len() / 2;
    let mut evals = [0u128; SWEEP_SPINE_SHIFT_ROUND_DEGREE + 1];
    let t_flat: [u128; SWEEP_SPINE_SHIFT_ROUND_DEGREE + 1] =
        std::array::from_fn(|k| tower_to_flat_u128(k as u128));
    ...
    for i in 0..half {
        ...
        for k in 0..=SWEEP_SPINE_SHIFT_ROUND_DEGREE {
            let t = t_flat[k];
    ...
    let evals_tower: [Block128; SWEEP_SPINE_SHIFT_ROUND_DEGREE + 1] =
        std::array::from_fn(|k| Block128::from(flat_to_tower_u128(evals[k])));
    RoundPolynomial::from_evals(&evals_tower)
}
...
mod tests {
    use super::*;
    use crate::circuit_sweep::N_SWEEP_SPINE_SLOTS;
    use crate::spine_mle_sweep::build_sweep_spine_unified_mle;
    ...
    fn random_mle(seed: u128) -> SweepSpineUnifiedMle {
        let state_ins: Vec<_> = (0..N_SWEEP_SPINE_SLOTS)
            .map(|i| random_state(i as u128 + seed))
            .collect();
        let (mle, _) = build_sweep_spine_unified_mle(&state_ins);
    ...
    fn round_degree_constant_matches() {
        assert_eq!(SWEEP_SPINE_UNIFIED_ROUND_DEGREE, 9);
    }
    ...
    fn flat_round_poly_matches_tower_round_poly() {
        ...
        let mut tow = build_unified_tables(&mle, &rho, N_SWEEP_SPINE_SLOTS);
        let mut flt =
            UnifiedFlatTables::from_tower(build_unified_tables(&mle, &rho, N_SWEEP_SPINE_SLOTS));
        let beta_flat = tower_to_flat_u128(beta.to_u128());
        let gamma_flat = tower_to_flat_u128(gamma.to_u128());
    ...
    fn honest_prover_verifies() {
        ...
        let (proof, _r_prime) = prove_sweep_spine_unified(&mle, &mut ch_p);
        assert_eq!(proof.round_polys.len(), N_SWEEP_SPINE_UNIFIED_VARS);

        let mut ch_v = Poseidon2bChannel::new();
        let red = verify_sweep_spine_unified(&proof, &mut ch_v).expect("verify must accept");
        assert_eq!(red.r_prime.len(), N_SWEEP_SPINE_UNIFIED_VARS);
    ...
    fn final_claims_match_native_mle_evaluations() {
        ...
        let mut ch_v = Poseidon2bChannel::new();
        let red = verify_sweep_spine_unified(&proof, &mut ch_v).unwrap();
        ...
        assert_eq!(
            evaluate_slice(&sweep_spine_permute_by_dec(&mle.s_in), &red.r_prime),
            red.s_in_dec_at_r
        );
        assert_eq!(
            evaluate_slice(&sweep_spine_permute_by_dec(&mle.s_out), &red.r_prime),
            red.s_out_dec_at_r
        );
        assert_eq!(
            evaluate_slice(&sweep_spine_permute_by_dec(&mle.state), &red.r_prime),
            red.state_dec_at_r
        );

        let s_out_dec_full = sweep_spine_permute_by_dec(&mle.s_out);
        let state_dec_full = sweep_spine_permute_by_dec(&mle.state);
        for j in 0..STATE_SIZE {
            assert_eq!(
                evaluate_slice(&sweep_spine_project_lane(&s_out_dec_full, j), &red.r_prime),
                red.s_out_lane_dec_at_r[j],
                "s_out lane {j}"
            );
            assert_eq!(
                evaluate_slice(&sweep_spine_project_lane(&state_dec_full, j), &red.r_prime),
    ...
    fn tampered_state_claim_is_rejected() {
        ...
        let mut ch_p = Poseidon2bChannel::new();
        let (mut proof, _r_prime) = prove_sweep_spine_unified(&mle, &mut ch_p);
        proof.state_at_r += Block128::ONE;

        let mut ch_v = Poseidon2bChannel::new();
        assert!(verify_sweep_spine_unified(&proof, &mut ch_v).is_none());
    }
    ...
    fn run_full_pipeline(seed: u128) -> (SweepSpineUnifiedMle, SweepSpineKillShotProof) {
        let mle = random_mle(seed);
        let mut ch = Poseidon2bChannel::new();
        let (main, r_prime) = prove_sweep_spine_unified(&mle, &mut ch);
        let (shift, _r2) = prove_sweep_spine_shift(&mle, &r_prime, &mut ch);
        (mle, SweepSpineKillShotProof { main, shift })
    }
    ...
    fn flat_shift_round_poly_matches_tower() {
        ...
        use rand::Rng;
        let mut rng = rand::thread_rng();
        let n: usize = 1 << N_SWEEP_SPINE_UNIFIED_VARS;
        let mut make =
    ...
    fn shift_gadget_round_count_and_degree() {
        ...
        assert_eq!(proof.shift.round_polys.len(), N_SWEEP_SPINE_UNIFIED_VARS);
        for p in &proof.shift.round_polys {
            assert!(p.degree() <= SWEEP_SPINE_SHIFT_ROUND_DEGREE);
        }
    }
    ...
    fn shift_gadget_verifies_end_to_end() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_sweep_spine_unified(&proof.main, &mut ch).expect("main verify");
        let shift_red =
            verify_sweep_spine_shift(&proof.shift, &main_red, &mut ch).expect("shift verify");
        assert_eq!(shift_red.r_double_prime.len(), N_SWEEP_SPINE_UNIFIED_VARS);
    }
    ...
    fn shift_final_claims_match_native_evaluations() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_sweep_spine_unified(&proof.main, &mut ch).unwrap();
        let shift_red = verify_sweep_spine_shift(&proof.shift, &main_red, &mut ch).unwrap();
    ...
    fn shift_tampered_final_claim_is_rejected() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_sweep_spine_unified(&proof.main, &mut ch).unwrap();
        assert!(verify_sweep_spine_shift(&proof.shift, &main_red, &mut ch).is_none());
    }
    ...
    fn shift_combined_target_matches_first_round_sum() {
        ...
        let mut ch_v = Poseidon2bChannel::new();
        let main_red = verify_sweep_spine_unified(&main, &mut ch_v).unwrap();
