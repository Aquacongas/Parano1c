...
/// One inverse witness, one product, and one equality pin for `lambda != 0`.
pub const ZK_OWNER_VERIFIER_LAMBDA_ADMISSIBILITY_ROWS: usize = 3;
...
pub struct ZkOwnerVerifierTraceInput {
    ...
    pub challenges_high_to_low: [LinExpr; ZK_AUTH_CAPSULE_BANK_VARS],
...
pub struct ZkOwnerVerifierTraceOutput {
    ...
    pub terminal_point: [LinExpr; ZK_AUTH_CAPSULE_BANK_VARS],
}
...
mod tests {
    ...
    use noid_gkr::zk_auth_capsule::{
        build_explicit_mlecheck_carrier, AuthCapsuleTerminalOperandClaims, ZkAuthCapsuleBankView,
    ...
    use super::super::{alloc_block, flat_of, test_support::tower_value};
    use super::*;
    ...
    fn point(domain: u128, salt: u128) -> [Block128; ZK_AUTH_CAPSULE_BANK_VARS] {
        std::array::from_fn(|index| elem(index + 23, domain, salt))
    }
    ...
    struct NativeCase {
        ...
        lambda: Block128,
        rounds: [ZkMleCheckRoundProof; ZK_AUTH_CAPSULE_BANK_VARS],
        challenges_high_to_low: [Block128; ZK_AUTH_CAPSULE_BANK_VARS],
    ...
    fn native_case(salt: u128) -> NativeCase {
        ...
        let mut lambda = elem(77, 0x1A4B_DA, salt ^ 0x66);
    ...
    fn alloc_input(b: &mut FieldR1csBuilder, case: &NativeCase) -> ZkOwnerVerifierTraceInput {
        ZkOwnerVerifierTraceInput {
            rho: std::array::from_fn(|index| alloc_block(b, case.rho[index])),
            mask_mle_at_input: alloc_block(b, case.mask_mle_at_input),
            mask_final_at_terminal: alloc_block(b, case.mask_final_at_terminal),
            lambda: alloc_block(b, case.lambda),
            rounds: std::array::from_fn(|round| ZkMleCheckRoundProofTrace {
                coeffs_without_constant: std::array::from_fn(|coefficient| {
                    alloc_block(b, case.rounds[round].coeffs_without_constant[coefficient])
                }),
            }),
            challenges_high_to_low: std::array::from_fn(|round| {
                alloc_block(b, case.challenges_high_to_low[round])
            }),
            terminal_operands: AuthCapsuleTerminalOperandClaimsTrace {
                increment: alloc_block(b, case.terminal_operands.increment),
                lane: std::array::from_fn(|lane| alloc_block(b, case.terminal_operands.lane[lane])),
            },
        }
    }
    ...
    struct BuiltCase {
        ...
        witness: Vec<F128>,
        trace_rows: usize,
        output_values: (Block128, Block128, Block128),
        terminal_point: [Block128; ZK_AUTH_CAPSULE_BANK_VARS],
    ...
    fn build_case(case: &NativeCase) -> Result<BuiltCase, ZkOwnerVerifierTraceError> {
        ...
        let lambda_wire = input_wire(&input.lambda);
        let expected_aliases: [LinExpr; ZK_AUTH_CAPSULE_BANK_VARS] =
        ...
        let output_values = (
            tower_value(&b, &output.expected_main_final),
            tower_value(&b, &output.main_eval),
            tower_value(&b, &output.mask_eval),
        );
        let terminal_point =
    ...
    fn owner_wrapper_matches_real_native_carrier_and_exact_ledger() {
        ...
        assert_eq!(ZK_OWNER_VERIFIER_LAMBDA_ADMISSIBILITY_ROWS, 3);
        assert_eq!(ZK_AUTH_TERMINAL_TRACE_ROWS, 167);
        assert_eq!(ZK_MLECHECK_VERIFIER_ROWS, 124);
    ...
    fn owner_wrapper_rejects_coeff_operand_mask_rho_and_challenge_tampering() {
        ...
        candidate.rounds[4].coeffs_without_constant[7] += Block128::ONE;
        assert_rejected(candidate, "round coefficient");

        let mut candidate = honest.clone();
        candidate.terminal_operands.lane[2] += Block128::ONE;
        assert_rejected(candidate, "terminal operand claim");

        let mut candidate = honest.clone();
        candidate.mask_mle_at_input += Block128::ONE;
        assert_rejected(candidate, "input mask claim");

        let mut candidate = honest.clone();
        candidate.mask_final_at_terminal += Block128::ONE;
        assert_rejected(candidate, "final mask claim");

        let mut candidate = honest.clone();
        candidate.rho[6] += Block128::ONE;
        assert_rejected(candidate, "rho");

        let mut candidate = honest;
        candidate.challenges_high_to_low[3] += Block128::ONE;
    ...
    fn owner_wrapper_rejects_lambda_zero_preflight_and_in_circuit() {
        ...
        tampered[built.lambda_wire] = F128::ZERO;
        assert!(
    ...
    fn owner_wrapper_preflight_is_atomic_for_constants() {
        ...
        input.lambda = LinExpr::constant(flat_of(native.lambda));
        let before = b.num_wires();
        assert_eq!(
        ...
        input.lambda = alloc_block(&mut b, native.lambda);
        input.terminal_operands.lane[3] =
            LinExpr::constant(flat_of(native.terminal_operands.lane[3]));
        let before = b.num_wires();
        assert_eq!(
            verify_zk_owner_trace(&mut b, &input),
            Err(ZkOwnerVerifierTraceError::DynamicInputIsConstant {
