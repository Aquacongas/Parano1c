...
//!   `h8` table;
//! - 64 fixed query units consume the returned query-bit aliases, the shared
...
use super::zk_query_carriers::{
    bind_zk_capsule_query_carriers, BoundZkCapsuleQueries, ZK_PATH_DIRECTION_BITS,
    ZK_QUERY_CARRIER_ROWS, ZK_QUERY_COUNT, ZK_QUERY_SEED_COUNT,
};
...
/// A 32-way cap MLE costs 31 products plus one root pin per digest lane.
pub const ZK_PHASE_B_SOURCE_CAP_ROWS_PER_QUERY: usize =
    ZK_PHASE_B_CAP_DIGEST_LANES * ((ZK_PHASE_B_SOURCE_CAP_NODES - 1) + 1);
/// A two-way cap MLE costs one product plus one root pin per digest lane.
...
const _: () = assert!(ZK_PHASE_B_SOURCE_CAP_NODES == 32);
const _: () = assert!(ZK_PHASE_B_MID_CAP_NODES == 2);
const _: () = assert!(ZK_PHASE_B_SOURCE_CAP_ROWS_PER_QUERY == 64);
const _: () = assert!(ZK_PHASE_B_MID_CAP_ROWS_PER_QUERY == 4);
...
pub struct ZkPhaseBCompositionTraceInput {
    ...
    /// Existing FF source-path `D[0..8]` cells, `[query][depth]`.
    pub source_path_directions: [[LinExpr; ZK_PATH_DIRECTION_BITS]; ZK_QUERY_COUNT],
    /// Existing FF mid-path `D[0..8]` cells, `[query][depth]`.
    ...
    pub upper: [LinExpr; UPPER_SYMBOLS],
...
fn preflight_dynamic_inputs(
    ...
    for (index, expression) in input.upper.iter().enumerate() {
        check_dynamic(expression, ZkPhaseBCompositionDynamicInput::Upper, index)?;
    }
    ...
    for (index, expression) in input.beta.iter().enumerate() {
        check_dynamic(expression, ZkPhaseBCompositionDynamicInput::Beta, index)?;
    }
    check_dynamic(
    ...
    for (index, expression) in input.tail.iter().enumerate() {
        check_dynamic(expression, ZkPhaseBCompositionDynamicInput::Tail, index)?;
    }
    check_dynamic(&input.gamma, ZkPhaseBCompositionDynamicInput::Gamma, 0)?;
    Ok(())
}
...
pub fn verify_zk_phase_b_composition_trace(
    ...
    let source_challenges: [LinExpr; SOURCE_STANDARD_FOLDS] =
        std::array::from_fn(|round| upper_link.beta[round].clone());
    let mid_challenges: [LinExpr; MID_STANDARD_FOLDS] =
    ...
    for query in 0..ZK_QUERY_COUNT {
        ...
        selected_mid_cap_roots.push(select_and_pin_cap(
            b,
            &input.mid_cap,
            std::slice::from_ref(&bound_queries.mid_cap_selectors[query]),
...
mod tests {
    ...
    use noid_fri_binius::zk_capsule_algebra::{
        ...
        evaluate_upper_at_low8, PHASE_B_HIGH_VARS,
    };
    ...
    fn native_case(salt: u128) -> NativeCase {
        ...
        let beta: [Block128; PHASE_B_LOW_VARS] =
            std::array::from_fn(|index| elem(index + 31, 0xBE7A, salt ^ 0x44));
        let phase_a_terminal_point: [Block128; OWNER_BANK_POINT_VARS] =
            std::array::from_fn(|index| elem(index + 47, 0x5A11, salt ^ 0x55));
        ...
        let tail: [Block128; TAIL_SYMBOLS] = tail_vec.try_into().expect("seven folds leave 16");
        assert_eq!(
        ...
        let high_point: &[Block128; PHASE_B_HIGH_VARS] = phase_a_terminal_point[PHASE_B_LOW_VARS..]
            .try_into()
            .expect("11-variable point has three HIGH coordinates");
        let low_point: &[Block128; PHASE_B_LOW_VARS] = phase_a_terminal_point[..PHASE_B_LOW_VARS]
            .try_into()
    ...
    fn alloc_input(b: &mut FieldR1csBuilder, native: &NativeCase) -> ZkPhaseBCompositionTraceInput {
        ZkPhaseBCompositionTraceInput {
            ...
            phase_a_terminal_point: std::array::from_fn(|index| {
                alloc_block(b, native.phase_a_terminal_point[index])
            }),
    ...
    fn build_case(native: &NativeCase) -> BuiltCase {
        ...
        let mid_leaf_wire = input_wire(&input.mid_leaves[7][native.queries[7] & 0xf]);
        ...
        let tail_wire = input_wire(&input.tail[7]);
        let upper_wire = input_wire(&input.upper[37]);
        let terminal_value_wire = input_wire(&input.terminal_oracle_value);
    ...
    fn complete_phase_b_matches_native_affine_code_caps_and_exact_ledger() {
        ...
        assert_eq!(ZK_PHASE_B_SOURCE_CAP_ROWS_PER_QUERY, 64);
        assert_eq!(ZK_PHASE_B_MID_CAP_ROWS_PER_QUERY, 4);
        assert_eq!(ZK_PHASE_B_COMPOSITION_TRACE_ROWS, 10_646);
    ...
    fn composition_dynamic_preflight_is_complete_and_atomic() {
        ...
        input.gamma = LinExpr::constant(flat_of(native.gamma));
        let before = b.num_wires();
        assert!(matches!(
