...
pub const ZK_PHASE_B_UPPER_EVAL_ROWS: usize = ZK_PHASE_B_UPPER_SYMBOLS - 1;
/// One multiplication per adjacent `tail16` pair at variable seven.
pub const ZK_PHASE_B_TAIL_LOCAL_FOLD_ROWS: usize = ZK_PHASE_B_H_SYMBOLS;
/// One complete three-variable MLE fold of `h8`.
pub const ZK_PHASE_B_H_EVAL_ROWS: usize = ZK_PHASE_B_H_SYMBOLS - 1;
/// One pin for `upper(s_low)=v` and one for `upper(beta)=h(s_high)`.
pub const ZK_PHASE_B_LINK_PIN_ROWS: usize = 2;
...
/// `255 + 1 + 255 + 1 + 1`.
pub const ZK_PHASE_B_ACTIVE_LINKAGE_ROWS: usize = 2 * ZK_PHASE_B_UPPER_EVAL_ROWS + 3;
...
const _: () = assert!(ZK_PHASE_B_UPPER_EVAL_ROWS == 255);
const _: () = assert!(ZK_PHASE_B_TAIL_LOCAL_FOLD_ROWS == 8);
const _: () = assert!(ZK_PHASE_B_H_EVAL_ROWS == 7);
const _: () = assert!(ZK_PHASE_B_UPPER_LINK_TRACE_ROWS == 527);
const _: () = assert!(ZK_PHASE_B_ACTIVE_LINKAGE_ROWS == 513);
const _: () = assert!(ZK_PHASE_B_UPPER_LINK_ROWS_OVER_ACTIVE == 14);
...
pub struct ZkPhaseBUpperLinkTraceInput {
    ...
    pub upper: [LinExpr; ZK_PHASE_B_UPPER_SYMBOLS],
    /// Shared Phase-A terminal point `[s_0, ..., s_10]`, LOW-to-HIGH.
    pub phase_a_terminal_point: [LinExpr; OWNER_BANK_POINT_VARS],
    /// Phase-B fold challenges `[beta_0, ..., beta_7]`, LOW-to-HIGH.
    pub beta: [LinExpr; PHASE_B_LOW_VARS],
    ...
    pub tail: [LinExpr; ZK_PHASE_B_TAIL_SYMBOLS],
}
...
pub struct ZkPhaseBUpperLinkTraceOutput {
    ...
    pub upper_at_phase_a_low: LinExpr,
    /// `MLE(upper, beta)`.
    pub upper_at_beta: LinExpr,
    /// `tail16` folded at adjacent pairs with `beta_7`.
    pub h: [LinExpr; ZK_PHASE_B_H_SYMBOLS],
    /// `MLE(h, s[8..11])`, pinned to `upper_at_beta`.
    pub h_at_phase_a_high: LinExpr,
}
...
pub fn verify_zk_phase_b_upper_link_trace(
    ...
    let upper_at_phase_a_low = mle_evaluate_small_trace(
        b,
        &input.upper,
        &input.phase_a_terminal_point[..PHASE_B_LOW_VARS],
    );
    debug_assert_eq!(b.num_wires() - trace_start, ZK_PHASE_B_UPPER_EVAL_ROWS);
    pin_eq(b, &upper_at_phase_a_low, &input.terminal_oracle_value);

    let upper_at_beta = mle_evaluate_small_trace(b, &input.upper, &input.beta);
    debug_assert_eq!(
        b.num_wires() - trace_start,
        2 * ZK_PHASE_B_UPPER_EVAL_ROWS + 1
    );

    // Variable seven is the LOWEST surviving tail variable, hence adjacent
    // pairs. Characteristic two gives `a + beta_7 * (a + b)`.
    let h: [LinExpr; ZK_PHASE_B_H_SYMBOLS] = std::array::from_fn(|index| {
    ...
    debug_assert_eq!(
        b.num_wires() - trace_start,
        2 * ZK_PHASE_B_UPPER_EVAL_ROWS + 1 + ZK_PHASE_B_TAIL_LOCAL_FOLD_ROWS
    );

    let h_at_phase_a_high =
        mle_evaluate_small_trace(b, &h, &input.phase_a_terminal_point[PHASE_B_LOW_VARS..]);
    pin_eq(b, &upper_at_beta, &h_at_phase_a_high);
...
mod tests {
    ...
    use crate::acceptance::trace::{
        alloc_block, alloc_blocks, const_block, test_support::tower_value,
    };
    ...
    const ZK_PHASE_B_DYNAMIC_INPUT_ROWS: usize = ZK_PHASE_B_UPPER_SYMBOLS
    ...
    #[derive(Clone)]
    struct NativeCase {
        upper: [Block128; ZK_PHASE_B_UPPER_SYMBOLS],
        phase_a_terminal_point: [Block128; OWNER_BANK_POINT_VARS],
        beta: [Block128; PHASE_B_LOW_VARS],
        terminal_oracle_value: Block128,
        tail: [Block128; ZK_PHASE_B_TAIL_SYMBOLS],
        h: [Block128; ZK_PHASE_B_H_SYMBOLS],
        upper_at_beta: Block128,
        h_at_phase_a_high: Block128,
    }
    ...
    fn native_case(salt: u128) -> NativeCase {
        let bank: [Block128; OWNER_BANK_RELATION_LEN] =
            std::array::from_fn(|index| elem(index, 0xB4A9_2048, salt ^ 0x11));
        let phase_a_terminal_point: [Block128; OWNER_BANK_POINT_VARS] =
            std::array::from_fn(|index| elem(index + 17, 0x5A11_0011, salt ^ 0x22));
        let beta: [Block128; PHASE_B_LOW_VARS] =
            std::array::from_fn(|index| elem(index + 41, 0xBE7A_0008, salt ^ 0x33));
        ...
        let tail: [Block128; ZK_PHASE_B_TAIL_SYMBOLS] =
    ...
    fn alloc_input(b: &mut FieldR1csBuilder, case: &NativeCase) -> ZkPhaseBUpperLinkTraceInput {
        ZkPhaseBUpperLinkTraceInput {
            upper: alloc_blocks(b, &case.upper)
                .try_into()
                .unwrap_or_else(|_| unreachable!("upper has a fixed array length")),
            phase_a_terminal_point: std::array::from_fn(|index| {
                alloc_block(b, case.phase_a_terminal_point[index])
            }),
            beta: std::array::from_fn(|index| alloc_block(b, case.beta[index])),
            terminal_oracle_value: alloc_block(b, case.terminal_oracle_value),
    ...
    fn build_case(case: &NativeCase) -> Result<BuiltCase, ZkPhaseBUpperLinkTraceError> {
        ...
        let upper_at_phase_a_low = tower_value(&b, &output.upper_at_phase_a_low);
        let upper_at_beta = tower_value(&b, &output.upper_at_beta);
        let h = std::array::from_fn(|index| tower_value(&b, &output.h[index]));
        let h_at_phase_a_high = tower_value(&b, &output.h_at_phase_a_high);
    ...
    fn zk_phase_b_upper_link_refuses_constants_before_appending_rows() {
        ...
        for (expected_input, expected_index, kind) in checks {
            ...
            match kind {
                0 => input.upper[expected_index] = const_block(native.upper[expected_index]),
                1 => {
                    input.phase_a_terminal_point[expected_index] =
                        const_block(native.phase_a_terminal_point[expected_index]);
                }
                2 => input.beta[expected_index] = const_block(native.beta[expected_index]),
                3 => {
                    input.terminal_oracle_value = const_block(native.terminal_oracle_value);
                }
                4 => input.tail[expected_index] = const_block(native.tail[expected_index]),
                _ => unreachable!(),
            }
            let before = b.num_wires();
            assert!(matches!(
    ...
    fn zk_phase_b_upper_link_has_exact_row_delta_and_content_invariant_shape() {
        ...
        assert_eq!(ZK_PHASE_B_UPPER_EVAL_ROWS, 255);
        assert_eq!(ZK_PHASE_B_TAIL_LOCAL_FOLD_ROWS, 8);
        assert_eq!(ZK_PHASE_B_H_EVAL_ROWS, 7);
        assert_eq!(ZK_PHASE_B_LINK_PIN_ROWS, 2);
        assert_eq!(ZK_PHASE_B_UPPER_LINK_TRACE_ROWS, 527);
        assert_eq!(ZK_PHASE_B_ACTIVE_LINKAGE_ROWS, 513);
        assert_eq!(ZK_PHASE_B_UPPER_LINK_ROWS_OVER_ACTIVE, 14);
