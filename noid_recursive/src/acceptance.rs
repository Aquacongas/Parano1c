...
//!
//! Strategy (chosen): **algebraic in-circuit + deferred-native FRI**. The
//! acceptance proof verifies the algebraic claims of the production `BlockProof`
//! (sumcheck/GKR reductions, composition, Poseidon2b permutations) inside a
//! boolean R1CS, and defers the FRI low-degree / Merkle-opening checks to a
//! single native check at the syncing tip via a running-hash commitment. This
//! matches the prior `RecursiveBlockAir` design and keeps per-block cost bounded.
//!
//! This module currently provides the strategy-independent boundary:
//!   * the [`AcceptanceProof`] envelope, and
...
//!
//! The in-circuit `r1cs_proof` and the `deferred_fri_commit` running hash are
//! filled by later slices.
