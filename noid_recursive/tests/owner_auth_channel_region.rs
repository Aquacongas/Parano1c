...
//! squeezes the same challenges in the flat basis. Two owner-auth shapes.

use noid_core::transcript::FiatShamir;
use noid_core::Block128;
use noid_gkr::owner_auth::{
    compute_owner_auth_boundary, owner_auth_gkr_channel, prove_owner_auth_killshot,
    verify_owner_auth_killshot_with_claims, OwnerAuthCircuit, OwnerAuthInputs, OwnerAuthLayout,
    OwnerAuthProofKillShot, OwnerAuthPublicInputs,
};
...
/// Honest owner-auth fixture (owners with secrets, addresses derived by the
/// native boundary computation). Secrets are used only to run the native
...
/// region chain squeezes the same challenges. Two owner-auth shapes (3 and 5
/// owners → different slot_bits / num_vars).
#[test]
fn owner_auth_schedule_matches_verify_killshot() {
    for (owner_count, seed) in [(3usize, 0xA3u128), (5, 0xB5)] {
        let (proof, public) = fixture(owner_count, seed);
        ...
        let claims = verify_owner_auth_killshot_with_claims(
            &proof,
            &circuit,
            &public,
            &mut real_channel,
        ...
        // both are in the post-step-6 state.
        {
            ...
            assert_eq!(
                ...
                "owners={owner_count}: schedule diverged from verify_owner_auth_killshot"
            );
        }
        ...
        for (k, (region, tower)) in cols.challenges.iter().zip(replay_challenges.iter()).enumerate() {
            assert_eq!(
                *region,
                flat_of_tower_u128(tower.0),
                "owners={owner_count} challenge {k} diverged"
            );
        }
        eprintln!(
            "[owner-auth-channel] owners={owner_count} nv={}: {} slots, {} data lanes, {} challenges",
        ...
        // Negative: flipping one absorbed data lane shifts the post-state.
        {
            ...
            assert_ne!(
                ...
                "owners={owner_count}: tampered lane left the challenge stream unchanged"
            );
        }
        ...
        // the transcript.
        {
            ...
            assert_ne!(
                bad_challenges.last(),
                replay_challenges.last(),
                "owners={owner_count}: tampered constant left the challenge stream unchanged"
            );
        }
    }
}
...
fn owner_auth_layout_shape_facts() {
    let (proof, public) = fixture(3, 0xFAC7);
    let schedule = owner_auth_channel_schedule(&proof, &public);
    let layout = compile_duplex(&schedule.ops);
    let num_vars = public.layout.num_vars;
