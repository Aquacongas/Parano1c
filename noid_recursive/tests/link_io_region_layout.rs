...
fn region_tail_layout_and_spec_are_consistent() {
    ...
    let frozen = vec![
        RegionFrozenClaim { slice: WitnessSlice { log2_len: 8, index: 40 }, arity: 8 },
        RegionFrozenClaim { slice: WitnessSlice { log2_len: 5, index: 41 }, arity: 5 },
        RegionFrozenClaim { slice: WitnessSlice { log2_len: 8, index: 42 }, arity: 8 },
        RegionFrozenClaim { slice: WitnessSlice { log2_len: 1, index: 43 }, arity: 1 },
    ];
    ...
    let layout = link_io_layout_region(
        k_log,
        block_bearing,
        Some(RegionIoShape { n_claims: frozen.len(), max_arity }),
    );
    assert_eq!(layout.region_tail_offset, base, "tail starts right after the base lanes");
    ...
    assert_eq!(spec.claims.len(), frozen.len(), "one IoClaimSpec per frozen claim");
    assert_eq!(spec.io_slice.index, 1, "io slice at the fixed dyadic position");
    ...
    for (ci, (fc, claim)) in frozen.iter().zip(spec.claims.iter()).enumerate() {
        ...
        assert_eq!(claim.point.len(), fc.slice.log2_len, "claim {ci} arity == slice vars");
        ...
        assert!(claim.point.start >= base, "claim {ci} point in tail (no recursion overlap)");
        assert!(claim.value < spec.io_len, "claim {ci} value within io_len");
        assert!(claim.point.end <= claim.value, "claim {ci} point ends before its value lane");
    }
    ...
    assert_eq!(d.region_tail_offset, base, "default tail offset == base len");
...
fn empty_region_shape_is_base_layout() {
    for k_log in [7usize, 22, 23] {
        for bb in [false, true] {
            ...
            let l = link_io_layout_region(k_log, bb, Some(RegionIoShape { n_claims: 0, max_arity: 8 }));
