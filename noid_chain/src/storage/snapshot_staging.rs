...
fn atomic_publish_segment(
    ...
    let result = (|| {
        ...
        //
        // Android app sandboxes can reject hard-link creation with EACCES
        // even when both paths are inside the application's private files
        // directory.  On Android use an atomic same-directory rename instead.
        //
        // The final path is checked first so an existing staged segment is
        // never intentionally replaced.  Snapshot staging has one local
        // session writer, so there is no legitimate competing publisher for
        // the same segment path.
        ...
        #[cfg(target_os = "android")]
        {
            match fs::symlink_metadata(&final_path) {
                Ok(_) => {
                    return Err(SnapshotStagingError::io(
                        ...
                        std::io::Error::new(
                            ...
                            "staged segment final path already exists",
                        ),
                    ));
                }
                ...
                Err(error) => {
                    return Err(SnapshotStagingError::io(
                        "inspect staged segment final path",
                        error,
                    ));
                }
            }
