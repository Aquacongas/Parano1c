...
foreach ($Binary in @("paranoid-gui.exe", "paranoid.exe")) {
...
$OutputBaseFilename = "paranoid-gui-v$Version-$Platform-setup"
...
try {
    ...
    $Node = Join-Path $InstallDir "paranoid.exe"
    ...
    foreach ($ForbiddenBinary in @("noid-cli.exe", "noid-extminer.exe")) {
